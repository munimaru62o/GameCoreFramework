...
- **完全な関心の分離**  
ACharacter や APlayerController へのハードコーディングを排除し、各機能を独立したコンポーネント（GameFeatures）として分割。既存のコードを汚染することなく、新機能やキャラクターを安全に追加・拡張できます。

- **魂と肉体の分離**  
PlayerState（永続的な魂）と Pawn（一時的な肉体）の責務を完全に分離。これによりAbilityやInputのバインディングを「永続的なもの」と「一時的なもの」に分解して定義することで、ポゼッション状態に左右されない柔軟なシステムを実現します。
...
- **[アクター制御システム (Interface駆動の意図伝達 & オプトイン設計)](Document/ja/Architecture/control_system.md)**  
  「プレイヤーの操作意図」と「Pawnの物理挙動」をインターフェースで完全に分離。入力側が意図をPushし、Pawnがキャッシュし、Mover等の物理エンジン側がPullして翻訳する「意図のバケツリレー」による高度な疎結合アーキテクチャを解説しています。
