...
public:
	 ...
	 */
	static void ReadyFeature(UObject* Implementer, AActor* Actor, FName FeatureName);
	static void ReadyFeature(UGameFrameworkComponentManager* GFCM, UObject* Implementer, AActor* Actor, FName FeatureName);

	/**
	 * Resets or sets a feature to the "Initial" state.
	 * Call this when a feature is invalidated or starting setup.
	 */
	static void InitFeature(UObject* Implementer, AActor* Actor, FName FeatureName);
	static void InitFeature(UGameFrameworkComponentManager* GFCM, UObject* Implementer, AActor* Actor, FName FeatureName);
	 ...
	 */
	static bool IsFeatureTag(AActor* Actor, FName FeatureName, FGameplayTag Tag);
	static bool IsFeatureTag(UGameFrameworkComponentManager* GFCM, AActor* Actor, FName FeatureName, FGameplayTag Tag);
};