...
#include "GCFSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EGCFAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
UENUM(BlueprintType)
enum class EGCFGamepadSensitivity : uint8
{
...
class UGCFLocalPlayer;

/**
 * UGCFSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UGCFSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UGCFSettingsShared, FOnSettingChangedEvent, UGCFSettingsShared* Settings);
...
public:
	...
	static UGCFSettingsShared* CreateTemporarySettings(const UGCFLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UGCFSettingsShared* LoadOrCreateSettings(const UGCFLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UGCFSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UGCFLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EGCFAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EGCFAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EGCFAllowBackgroundAudioSetting AllowAudioInBackground = EGCFAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EGCFGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EGCFGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EGCFGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EGCFGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EGCFGamepadSensitivity GamepadLookSensitivityPreset = EGCFGamepadSensitivity::Normal;
	UPROPERTY()
	EGCFGamepadSensitivity GamepadTargetingSensitivityPreset = EGCFGamepadSensitivity::Normal;
