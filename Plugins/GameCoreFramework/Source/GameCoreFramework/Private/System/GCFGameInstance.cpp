...
#include "System/GCFGameInstance.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GCFGameInstance)

namespace GCF
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarGCFTestEncryption(
		TEXT("GCF.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarGCFUseDTLSEncryption(
		TEXT("GCF.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using GCF.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarGCFTestDTLSFingerprint(
		TEXT("GCF.TestDTLSFingerprint"),
...
UGCFGameInstance::UGCFGameInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UGCFGameInstance::Init()
{
	Super::Init();
	...
	{
		ComponentManager->RegisterInitState(GCFGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(GCFGameplayTags::InitState_DataAvailable, false, GCFGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(GCFGameplayTags::InitState_DataInitialized, false, GCFGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(GCFGameplayTags::InitState_GameplayReady, false, GCFGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UGCFGameInstance::OnPreClientTravelToSession);
	}
}

void UGCFGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
AGCFPlayerController* UGCFGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AGCFPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UGCFGameInstance::CanJoinRequestedSession() const
{
...
void UGCFGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UGCFLocalPlayer* LocalPlayer = Cast<UGCFLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void UGCFGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (GCF::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (GCF::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("GCFTest.pem");
...
void UGCFGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UGCFGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (GCF::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (GCF::bUseDTLSEncryption)
		{
