...
#include "AbilitySystem/Execution/GCFDamageExecution.h"
...
// #include "Teams/GCFTeamSubsystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GCFDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UGCFCombatAttributeSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UGCFDamageExecution::UGCFDamageExecution()
{
	RelevantAttributesToCapture.Add(DamageStatics().BaseDamageDef);
}


void UGCFDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FGCFGameplayEffectContext* TypedContext = FGCFGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	/*if (HitActor)
	{
		UGCFTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UGCFTeamSubsystem>();
	...
	{
		UE_LOG(LogGCFAbilitySystem, Error, TEXT("Damage Calculation cannot deduce a source location for damage coming from %s; Falling back to WORLD_MAX dist!"), *GetPathNameSafe(Spec.Def));
	}

	// Apply ability source modifiers
	float PhysicalMaterialAttenuation = 1.0f;
	float DistanceAttenuation = 1.0f;
	if (const IGCFAbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UGCFHealthAttributeSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
