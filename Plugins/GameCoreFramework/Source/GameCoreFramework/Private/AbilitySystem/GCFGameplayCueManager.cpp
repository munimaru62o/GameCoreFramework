...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GCFGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EGCFEditorLoadMode
{
...
namespace GCFGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("GCF.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via GCFGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UGCFGameplayCueManager::DumpGameplayCues));

	static EGCFEditorLoadMode LoadMode = EGCFEditorLoadMode::LoadUpfront;
}
...
UGCFGameplayCueManager::UGCFGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UGCFGameplayCueManager* UGCFGameplayCueManager::Get()
{
	return Cast<UGCFGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UGCFGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UGCFGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogGCFAbilitySystem, Warning, TEXT("UGCFGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UGCFGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (GCFGameplayCueManagerCvars::LoadMode)
	{
	case EGCFEditorLoadMode::LoadUpfront:
		return true;
	case EGCFEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EGCFEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UGCFGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UGCFGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UGCFGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UGCFGameplayCueManager* GCM = Cast<UGCFGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogGCFAbilitySystem, Error, TEXT("DumpGameplayCues failed. No UGCFGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogGCFAbilitySystem, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogGCFAbilitySystem, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogGCFAbilitySystem, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogGCFAbilitySystem, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogGCFAbilitySystem, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogGCFAbilitySystem, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogGCFAbilitySystem, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogGCFAbilitySystem, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogGCFAbilitySystem, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogGCFAbilitySystem, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogGCFAbilitySystem, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogGCFAbilitySystem, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UGCFGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UGCFGameplayCueManager* StrongThis = Get())
					{
...
void UGCFGameplayCueManager::HandlePostGarbageCollect()
{
...
void UGCFGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogGCFAbilitySystem, Warning, TEXT("UGCFGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UGCFGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (GCFGameplayCueManagerCvars::LoadMode)
	{
	case EGCFEditorLoadMode::LoadUpfront:
...
void UGCFGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UGCFGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UGCFGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UGCFGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UGCFGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UGCFGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
